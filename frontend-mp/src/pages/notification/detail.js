...
Page({
  ...
  loadNotificationDetail: function() {
    ...
    wx.request({
      url: `${app.globalData.apiBaseUrl}/notifications/${this.data.id}`,
      method: 'GET',
      header: {
      ...
      success: (res) => {
        if (res.statusCode === 200 && res.data.success) {
          ...
          this.setData({
            error: res.data.message || '获取通知详情失败',
            loading: false
          });
        }
      },
  ...
  loadComments: function() {
    ...
    wx.request({
      url: `${app.globalData.apiBaseUrl}/notifications/${this.data.id}/comments`,
      method: 'GET',
      header: {
        'Authorization': `Bearer ${wx.getStorageSync('token')}`
      },
  ...
  likeComment: function(e) {
    ...
    // 调用API更新点赞状态
    wx.request({
      url: `${app.globalData.apiBaseUrl}/notifications/comments/${commentId}/like`,
      method: 'POST',
      header: {
        'Authorization': `Bearer ${wx.getStorageSync('token')}`
      },
  ...
  submitComment: function() {
    ...
    wx.request({
      url: `${app.globalData.apiBaseUrl}/notifications/comments`,
      method: 'POST',
      data: data,
      header: {
        'Authorization': `Bearer ${wx.getStorageSync('token')}`
  ...
  onShareAppMessage: function() {
    const title = this.data.notification ? this.data.notification.title : '通知详情';
    return {
      title: title,
      path: `/src/pages/notification/detail?id=${this.data.id}`
    };
  }
}); 